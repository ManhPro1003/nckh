...
# ==========================================
# 1. THUẬT TOÁN MẠCH PCB (BẢN CHUẨN CỦA BẠN)
# ==========================================
def process_pcb(gray_img, scale, target_width_mm, feedrate, thresh_val, pen_size_mm, z_up, z_down):
    ...
    # Bước 1: Vẽ viền ngoài cực kỳ chính xác
        ...
        for x in range(w):
            if row[x] == 255:
                ...
                if start_x != -1:
                    ...
                    start_x = -1
        if start_x != -1 and (w - 1 - start_x) > 1:
...
def process_image(img_path, main_mode, art_style, target_width_mm, feedrate, thresh_val, smooth_val, density, pen_size_mm, z_up, z_down, g_scale):
    ...
    # ----------------------------------------------------
    # ĐIỂM SÁNG TẠO: TÍNH TOÁN KÍCH THƯỚC SAU KHI PHÓNG ĐẠI
    # Mọi thứ (G-code, SVG, Lưới tọa độ) đều sẽ tự động đồng bộ theo actual_width_mm
    # ----------------------------------------------------
    actual_width_mm = target_width_mm * (g_scale / 100.0)
    scale = actual_width_mm / gray.shape[1]
    
    if main_mode == 'pcb': 
        return process_pcb(gray, scale, actual_width_mm, feedrate, thresh_val, pen_size_mm, z_up, z_down)
    if main_mode == 'art' and art_style == 'spiral': 
        return process_spiral(gray, scale, actual_width_mm, feedrate, density, pen_size_mm, z_up, z_down)
    if main_mode == 'art' and art_style == 'sketch': 
        return process_sketch(gray, scale, actual_width_mm, feedrate, density, thresh_val, pen_size_mm, z_up, z_down)
